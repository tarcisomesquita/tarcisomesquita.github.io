// clear ; gcc -W -Wall -lpthread -o httpd.elf httpd.c && ./httpd.elf ; echo $?
...
int startup(u_short *port) {
   ...
   if (*port == 0) {
      ...
      if (getsockname(httpd, (struct sockaddr *)&name, (unsigned int *)&namelen) == -1) error_die("getsockname");
...
void serve_file(int client, const char *filename) {
   ...
   buf[0] = 'A'; buf[1] = '\0';
   while ((numchars > 0) && strcmp("\n", buf)) numchars = get_line(client, buf, sizeof(buf)); // read & discard headers
...
int main() {
   ...
   while (1) {
      client_sock = accept(server_sock, (struct sockaddr *)&client_name, (unsigned int *)&client_name_len);
