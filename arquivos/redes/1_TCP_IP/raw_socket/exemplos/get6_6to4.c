...
unsigned short int tcp6_checksum (struct ip6_hdr, struct tcphdr, unsigned char *, int);
...
// Maximum TCP payload size = 65535 - IPv6 header (40 bytes) - TCP header (20 bytes)
  tmp = (char *) malloc (IP_MAXPACKET - IP6_HDRLEN - TCP_HDRLEN);
  if (tmp != NULL) {
    payload = tmp;
  } else {
  ...
  memset (payload, 0, IP_MAXPACKET - IP6_HDRLEN - TCP_HDRLEN);
...
// Total length of datagram (16 bits): IPv6 payload length + IPv4 header + IPv6 header
  ip4hdr->ip_len = htons (TCP_HDRLEN + payloadlen + IP4_HDRLEN + IP6_HDRLEN);
...
// TCP checksum (16 bits)
  tcphdr->th_sum = tcp6_checksum (*ip6hdr, *tcphdr, (unsigned char *) payload, payloadlen);

// TCP data
  memcpy (outpack + IP4_HDRLEN + IP6_HDRLEN + TCP_HDRLEN, payload, payloadlen);

  memset (&sin, 0, sizeof (sin));
  sin.sin_family = AF_INET;
  sin.sin_addr.s_addr = ip4hdr->ip_dst.s_addr;

  if (sendto (sd, outpack, IP4_HDRLEN + IP6_HDRLEN + TCP_HDRLEN + payloadlen, 0, (struct sockaddr *) &sin, sizeof (struct sockaddr)) < 0)  {
...
unsigned short int
tcp6_checksum (struct ip6_hdr iphdr, struct tcphdr tcphdr, unsigned char *payload, int payloadlen)
{
