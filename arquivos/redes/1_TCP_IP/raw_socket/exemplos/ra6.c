...
// Obtain advertising router (source) MAC address.
...
// Append options to end of router advertisement struct.
  memcpy (outpack + RA_HDRLEN, options, optlen);
...
// psdhdr[16 to 31] = destination IPv6 address, set earlier.
  ...
  memcpy (psdhdr + 40, outpack, RA_HDRLEN + optlen);
  ra->nd_ra_hdr.icmp6_cksum = checksum ((unsigned short int *) psdhdr, psdhdrlen);
