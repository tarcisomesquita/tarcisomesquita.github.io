...
/*
'Create form', 'buildForm_')
Generate test data', 'generateTestData_')
'Assign activities', 'assignActivities_')
...
let sectionHelpText = Utilities.formatString('Please choose your top %d activities', NUM_ITEMS_TO_RANK);
form.addSectionHeaderItem().setTitle('Activity choices').setHelpText(sectionHelpText);

// Presents activity ranking as a form grid with each activity as a row and rank as a column.
let rows = loadActivitySchedule_(ss).map(function(activity) {return activity.description;});
let columns = range_(1, NUM_ITEMS_TO_RANK).map(function(value) {return Utilities.formatString('%s', toOrdinal_(value));});
...
function findResponseSheetForForm_(ss) {
  ...
  if (!ss || !formUrl) return undefined;
  
  let sheets = ss.getSheets();
  for (let i in sheets) {
    if (sheets[i].getFormUrl() === formUrl) return sheets[i];
  }
  return undefined;
}
