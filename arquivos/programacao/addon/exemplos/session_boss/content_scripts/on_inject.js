...
//          .then(() => log.info(window.location.href + " init done") )
    ...
    function setPageMessage(msg) {
        if (window.location.href == "about:blank") {
            ...
            div.textContent = "msg - " + msg;           // use textContent to escape any unsafe text in msg.
    ...
    function showErrorMessage(errmsg, url) {
        if (window.location.href == "about:blank") {
            QS("head").innerHTML =                      // innerHTML used here is for setting the style text (safe text).
                `
                <style type='text/css'>
                  html {
                    font-size: 16px;
            ...
            `;
            // .innerHTML() is used only for the template text.  The potentially unsafe text are set via .text() and .val() below.
            QS("body").innerHTML = `
                <div class='warning'>
            ...
            `;
            QS("#param-errmsg").textContent = errmsg;
            QS("#param-url").value = url;
            QS("#param-url").focus();
            QS("#param-url").select();
        }
    }
