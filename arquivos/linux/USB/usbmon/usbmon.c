...
void Usage(void) {
  fprintf(stderr, "Usage: usbmon [-i usbN] [-f0|-fu|-fh] [-a0|-a1|-am] [-s len]\n");
  exit(2);
}
...
int main(int argc , char **argv) {
  ...
  if (sizeof(struct usbmon_packet) != 48) {
    printf("\nERRO: sizeof(struct usbmon_packet) != 48\n\n");
    return 1;
  }
  if (sizeof(struct usbmon_packet_1) != 64) {
    printf("\nERRO: sizeof(struct usbmon_packet_1) != 64\n\n");
    return 1;
  }
...
void parse_params(struct params *p, char **argv) {
  ...
  while ((arg = *argv++) != NULL) {
    ...
    case 'i':
      if (arg[2] != 0) Usage();
      if ((arg = *argv++) == NULL) Usage();
      if (strncmp(arg, "usb", 3) == 0) arg += 3;
      if (!isdigit(arg[0])) Usage();
      errno = 0;
