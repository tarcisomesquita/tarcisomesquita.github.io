...
int print_super_block(unsigned long int grupo) {
   int i, n;
   
   if ( grupo >= (blocks_count / blocks_per_group)) { printf("\nERRO: O sistema não tem o grupo %lu.\n\n", grupo); return 1; }
   if ( grupo != 0 && grupo != 1) {
      for (i = 1; ; i++) {
         if (grupo == pow(3, i)) break;
         if (grupo == pow(5, i)) break;
         if (grupo == pow(7, i)) break;
         if ((unsigned long int)pow(3, i) >= grupo) { printf("\nERRO: O grupo %lu não tem backup do super bloco.\n\n", grupo); return 1; }
      }
   }
   
   //The groups chosen are 0, 1 and powers of 3, 5 and 7. 
   
   n = lseek(fd, 1024 + grupo * blocks_per_group * block_size, SEEK_SET); // posiciona no início do super bloco
   if (n != (1024 + grupo * blocks_per_group * block_size)) { printf("\nERRO: lseek.\n\n"); return 1; }
   ...
   printf("\n");
   printf("Super bloco do grupo %lu\n", grupo);
   printf("\n");
