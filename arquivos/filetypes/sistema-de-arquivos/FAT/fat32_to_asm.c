...
//   struct msdos_dir_entry *dir;
   ...
   printf("ignored:       db 0x%02X, 0x%02X, 0x%02X   ; 0x%08X   Boot strap short or near jump.  0xEB 0x3C   JMP SHORT x3E\n", 
   ...
   printf("sec_per_clus:  db \033[33m0x%02X\033[m               ; 0x%08X   sectors per cluster. 1, 2, 4, 8, 16, 32, 64.\n",
   ...
   printf("sectors:       dw \033[33m0x%04X\033[m             ; 0x%08X   number of sectors (use 4 byte value at offset 0x20)",
           *dw, offset);
   offset = offset + 2;
   if (*dw != 0x0000) printf("  . . . . . . . . . . . . . . . . . [\033[31mFL\033[m]\n");
   else               printf("  . . . . . . . . . . . . . . . . . [\033[32mOK\033[m]\n");
   ...
   offset = offset + 2;
   if (boot->secs_track != 0x003E) printf("  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [\033[31mFL\033[m]\n");
   else                            printf("  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [\033[32mOK\033[m]\n");
   
   printf("heads:         dw 0x%04X             ; 0x%08X   number of heads",   boot->heads, offset);
   offset = offset + 2;
   if (boot->heads != 0x00F6) printf("  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [\033[31mFL\033[m]\n");
   else                       printf("  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [\033[32mOK\033[m]\n");
...
*/   
   ...
   printf("trail_sig:     dd 0xAA550000         ; 0x%08X" , offset);
   offset = offset + 4;
   if (fsinfo->reserved2[3] != 0xAA550000) printf("  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [\033[31mFL\033[m]\n");
   else                                    printf("  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . [\033[32mOK\033[m]\n");

/*
x3e:             push cs         ; 0x3e  448       Operating system boot code
                 pop ds
                 mov si,x5B
...
x5B:             db "This is not a bootable disk.  Please insert a bootable floppy and", 0x0d, 0x0a, "press any key to try again", 0x0d, 0x0a, 0x00
*/
...
hexdump -C fat32.bin
00024000  36 34 4d 69 42 20 20 20  20 20 20 08 00 00 3c 65  |64MiB      ...<e|
00024010  54 49 54 49 00 00 3c 65  54 49 00 00 00 00 00 00  |TITI..<eTI......|
00024020  00 00 00 00 00 00 00 00  00 00 00 00 00 00 00 00  |................|

 sudo mkdir /media/ubuntu/64MiB/TESTE

00024000  36 34 4d 69 42 20 20 20  20 20 20 08 00 00 3c 65  |64MiB      ...<e|
00024010  54 49 54 49 00 00 3c 65  54 49 00 00 00 00 00 00  |TITI..<eTI......|
00024020  54 45 53 54 45 20 20 20  20 20 20 10 00 00 d7 66  |TESTE      ....f|
00024030  54 49 54 49 00 00 d7 66  54 49 03 00 00 00 00 00  |TITI...fTI......|
00024040  00 00 00 00 00 00 00 00  00 00 00 00 00 00 00 00  |................|
*
00025000  2e 20 20 20 20 20 20 20  20 20 20 10 00 00 d7 66  |.          ....f|
00025010  54 49 54 49 00 00 d7 66  54 49 03 00 00 00 00 00  |TITI...fTI......|
00025020  2e 2e 20 20 20 20 20 20  20 20 20 10 00 00 d7 66  |..         ....f|
00025030  54 49 54 49 00 00 d7 66  54 49 00 00 00 00 00 00  |TITI...fTI......|
